...
// Zod schema for polished message output
const PolishedMessageSchema = z.object({
  body: z.string(),
...
export class MessagePolisherAgent {
  /**
   * Polishes an existing message based on user feedback while maintaining personalization.
   */
  async polishMessage(
    options: PolishOptions,
    apiKey: string,
    modelName: string,
  ): Promise<{ body: string; wordCount: number; changes: string[] }> {
...
export const messagePolisherAgent = new MessagePolisherAgent();